...
class RRT:
    # Constructor
    def __init__(self, map_array, start, goal, prob_map=None):
        self.map_array = map_array  # map array, 1->free, 0->obstacle
        self.size_row = map_array.shape[0]  # map size
        self.size_col = map_array.shape[1]  # map size
        self.num_pixels = self.size_row * self.size_col

        self.start = Node(start[0], start[1])  # start node
        self.goal = Node(goal[0], goal[1])  # goal node
        self.vertices = []  # list of nodes
        self.found = False  # found flag
        self.save_path = None  # save the path
        self.prob_map = prob_map
        self.mode = 'type1'
        if self.prob_map is not None:
            self.sum_prob_map = self.prob_map.ravel().sum()
            self.flttn_prob_map = self.prob_map.ravel()
    ...
    def get_new_point(self, goal_bias):
        ...
        #     sample = np.random.choice(self.num_pixels, size=1, p=self.flttn_prob_map/self.sum_prob_map)
        #     point = [sample[0]%self.size_col, sample[0]//self.size_row]
        # else:
        ...
        #         point = [sample[0]%self.size_col, sample[0]//self.size_row]
        #     else:
        #         if np.random.random() < goal_bias+0.7:
        #             point = [self.goal.row, self.goal.col]
        #         # or generate a random point
        #         else:
        #             point = [np.random.randint(0, self.size_col - 1), np.random.randint(0, self.size_row - 1)]
    ...
    def extend(self, extend_dis=10, goal_bias=0.05):
        ...
        if (0 <= new_row < self.size_row) and (0 <= new_col < self.size_col) and \
            ...
            if self.prob_map[new_node.col][new_node.row] == 1:
                self.mode = 'type2'
    ...
    def draw_map(self):
        ...
        for node in self.vertices[1:-1]:
            draw.ellipse([(node.row - radius, node.col - radius), (node.row + radius, node.col + radius)], fill='blue')
            draw.line([(node.row, node.col), (node.parent.row, node.parent.col)], fill='blue')
        if self.found:
            cur = self.goal
            while cur.col != self.start.col or cur.row != self.start.row:
                draw.line([(cur.row, cur.col), (cur.parent.row, cur.parent.col)], fill='purple')
                cur = cur.parent
                draw.ellipse([(self.start.row - radius, self.start.col - radius),
                              (self.start.row + radius, self.start.col + radius)], fill='purple')

        radius = 3
        draw.ellipse([(self.start.row - radius, self.start.col - radius),
                      (self.start.row + radius, self.start.col + radius)], fill='green')
        draw.ellipse([(self.goal.row - radius, self.goal.col - radius),
                      (self.goal.row + radius, self.goal.col+ radius)], fill='red')
    ...
    def draw_map2(self):
        ...
        # save image
        if self.save_path is not None:
            plt.savefig(self.save_path,bbox_inches='tight', pad_inches = -0.1)
        # show image
        # plt.show()
